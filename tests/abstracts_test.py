...
class TestNumericSignal1D(unittest.TestCase):
    ...
    def test_get_peak_between(self):
        signal = abstracts.ContinuousSignal1D.from_csv('tests/data/table_2.csv', detect_axis_name_and_unit=False, detect_value_name_and_unit=False)
        self.assertEqual(signal.get_peak_between(1, 2), (1.1, 2))
        
    def test_preview(self):
        signal = abstracts.ContinuousSignal1D.from_csv(
...
class TestFractionSignal(unittest.TestCase):
    ...
    def test_export(self):
        ...
        with self.assertRaises(Exception):
            fraction_signal.export("tests/out/fraction_1_export_with_FractionSignal.csv")
        fraction_signal.export("tests/out/fraction_1_export_with_FractionSignal.csv", mode="replace")
